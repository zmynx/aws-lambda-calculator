...
def calc_cost(duration: int,
              requests_in_millions: int,
              concurrency: int,
              ram: float):

    requests = requests_in_millions * 1000000
    compute = requests * duration * 0.001
    logging.info(f'{requests} requests x {duration} ms x 0.001 ms to sec conversion factor = {compute} total compute (seconds)')
    ...
    logging.info(f'{compute_gb} GB-s - {free_tier_gb} free tier GB-s = {compute_gb - free_tier_gb} GB-s')
    total_compute_gb = compute_gb - free_tier_gb
    logging.info(f'Max ({total_compute_gb} GB-s, 0 ) = {max(total_compute_gb, 0)} total billable GB-s')
    ...
    logging.info(f'{requests} requests - {free_tier_requests} free tier requests = {total_requests} monthly billable requests')
    total_requests_cost = total_requests * 0.0000002
    logging.info(f'{total_requests} total monthly billable requests x 0.0000002 USD = {total_requests_cost} USD (monthly request charges)')
...
if __name__ == '__main__' :
    ...
    requests = 2
    concurrency = 1
    calc_cost(duration=duration, requests_in_millions=requests, concurrency=concurrency, ram=ram)